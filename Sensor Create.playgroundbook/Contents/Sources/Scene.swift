...
public class Scene: PlaygroundRemoteLiveViewProxyDelegate {
    ...
    func handleSceneTouchEvent(touch: Touch) {
        ...
        // Look to see if there was a touched graphic and if there are any tapHandlers assigned for that graphic. Assign those to a handler that will be called once all graphics have been examined.
        ...
        if let touchedGraphic = touch.touchedGraphic {
            if let button = placedGraphics[touchedGraphic.id] as? Button {
                if touch.firstTouch {
                    tapHandler = button.onTapHandler
            ...
            } else if let sprite = placedGraphics[touchedGraphic.id] as? Sprite {
                if touch.firstTouch {
                    tapHandler = sprite.onTapHandler
                }

            } else if let label = placedGraphics[touchedGraphic.id] as? Label {
                if touch.firstTouch {
                    tapHandler = label.onTapHandler
                }
                
            } else if let player = placedGraphics[touchedGraphic.id] as? Player {
                if touch.firstTouch {
                    tapHandler = player.onTapHandler
                }
                
            } else if let graphic = placedGraphics[touchedGraphic.id] {
                if touch.firstTouch {

...
public extension Scene {
    ...
    public func createBackgroundUI(background1: Color, background1Gradient: Color, background2: Color, background2Gradient: Color, dots: Color, displayGraphic: Image, displayGraphicName: String, dotHeight: Double = -100) {
        ...
        var dotPlacement: Point = Point(x: -250, y: dotHeight)
        for _ in 1...11 {
    ...
    @discardableResult public func createStartButton() -> Button {
        let startButton = Button(type: .red, text: NSLocalizedString("PLAY", comment: "Play game button"), name: Display.play)
    ...
    @discardableResult public func createPracticeButton(handler: @escaping () -> Void) -> Button {
        let practiceButton: Button = Button(type: .red, text: NSLocalizedString("PRACTICE", comment: "Practice game button"), name: Display.title)
        ...
        practiceButton.setOnTapHandler(handler)
        practiceButton.disablesOnDisconnect = true
        self.place(practiceButton, at: Point(x: 150, y: -300))
        return practiceButton
    }
    
    @discardableResult public func createPlayButton(text: String, handler: @escaping () -> Void) -> Button {
        let playButton: Button = Button(type: .red, text: NSLocalizedString("\(text)", comment: "Ready to play game button"), name: Display.title)
        ...
        playButton.setOnTapHandler(handler)
        playButton.disablesOnDisconnect = true
        self.place(playButton, at: Point(x: -150, y: -300))
        return playButton
    }
    
    public func createTitleText(title: String, description: String) {
    ...
    public func removeTitleDisplay() {
        self.removeGraphics(named: Display.title)

    }

    public func removePlayDisplay() {
        self.removeGraphics(named: Display.play)
    }
    
    public func createDisplayLabel(backgroundColor: Color, alpha: Double, position: Point, labelTitle: String, startingText: String = "", name: String = "displayLabel", backgroundName: String = "displayLabel", labelTextColor: Color = .white) -> Label {
    ...
    public func countDown(from value: Int = 3, completion: @escaping () -> Void) {

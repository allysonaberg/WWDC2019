...
private class BackgroundContainerNode : SKSpriteNode {
    ...
    private func gridAxislabel(_ text: String) -> SKNode {
        let axisLabelAttributes: [NSAttributedString.Key: Any] = [
...
public class LiveViewScene: SKScene, UIGestureRecognizerDelegate, SKPhysicsContactDelegate, GraphicAccessibilityElementDelegate, BackgroundAccessibilityElementDelegate {
    ...
    public var lightSensorImage: UIImage? {
        didSet {
            if let lsImage = lightSensorImage, !blockLightSensorImage {
...
//        view.showsPhysics = true
    ...
    func handleTouch(at: CGPoint, firstTouch: Bool = false, ignoreNode: Bool = false) {
        ...
        if !ignoreNode {
            var node: SKNode?
            let nodes = containerNode.nodes(at: at)
            var interactableLiveGraphics: [LiveViewGraphic] = []
            for node in nodes {
                if let id = node.name, let graphic = graphicsInfo[id], graphic.allowsTouchInteraction {
                    interactableLiveGraphics.append(graphic)
                    break
                }
            }
            node = interactableLiveGraphics.first?.backingNode
    ...
    func setSceneBackgroundColor(color: UIColor) {
        DispatchQueue.main.async { [unowned self] in
            ...
            self.sceneDelegate?.lightSensorImage = nil

...
/// ToneSensor tells the iPad to start collecting data from the microphone.
///
/// - localizationKey: ToneSensor
public class ToneSensor : AURenderCallbackDelegate {
    ...
    var axToneOutput = ToneOutput()
    
    /// Initialize a ToneSensor.
    ///
    /// - localizationKey: ToneSensor()
    public init() {
        var status = noErr
        ...
        AVAudioSession.sharedInstance().requestRecordPermission () { allowed in
            if allowed {
                ...
                DispatchQueue.main.async {
                    // signal to the live view that a tone sensor is active
                    Message.setAXUITone(self.currentSample).send()
                    
                    Scene.axUIToneSensor = self
                }
    ...
    var axUITone = Tone(pitch: 0.0, volume: 0.0) {
        didSet {
            axOverrideTimer?.invalidate()
            
            axToneOutput.play(tone: axUITone)
